...
public class CollaboratorController {
	...
	public ResponseEntity<Response> addCollaborator(@Valid @RequestBody CollaboratorDto collaboratorDto,
			@PathVariable("id") long noteId, @RequestHeader("token") String token, BindingResult bindingResult) {
	...
	public ResponseEntity<Response> deleteCollaborator(@PathVariable(value = "id") Long noteId,
			@RequestHeader("token") String token, @RequestHeader("collaboratorId") Long cId) {
		int i = collaboratorService.deleteCollaborator(cId, token, noteId);
		return i == 1 ? ResponseEntity.status(HttpStatus.CREATED).body(new Response("deleted successfully", 200, null))
	...
	public ResponseEntity<Response> getAllCollaborator(@PathVariable(value = "id") Long noteId,
			@RequestHeader("token") String token) {
		List<Collaborator> collList = collaboratorService.getAllNoteCollaborators(token, noteId);
		return collList != null
				? ResponseEntity.status(HttpStatus.CREATED)
						.body(new Response("all note collaborators are", 200, collList))
				: ResponseEntity.status(HttpStatus.BAD_REQUEST).body(new Response("Something went wrong", 400));
	}
